...
Buggy code in a driver or a module may lead to one of the system faults states : \textbf{kernel oops} and \textbf{system hang}.\\
\vspace{8px}
...
\begin{figure}[H]
    ...
    \end{figure}		
		...
		\begin{figure}[H]
			\centering
        	\includegraphics[scale=0.55]{img/solution/oops-error-kernel-smp.png}
...
#define GPIO_BASE			(BCM2708_PERI_BASE + 0x200000)
		...
		\begin{figure}[H]
			\centering
        	\includegraphics[scale=0.42]{img/solution/jtag-enabler-raspberry-pi-3.png}
...
\textbf{\color{red}Important : } JTAG is enabled on Raspberry PI 3.	
		...
		\begin{figure}[H]
			\centering
        	\includegraphics[scale=0.37]{img/solution/openocd-bcm2837-launched.png}
...
0xba00, ver : 0x4)} \fg means that OpenOCD was able to detect the Raspberry PI 3. We also see the breakpoints which indicates highly that the connection was a success.
	...
	\item \textbf{OpenOCD made easy with OESdebug : } OpenOCD is quite difficult and complex to setup. We have provided a tool called \og OpenOCD-wrapper \fg (OpenEasy Debug is written in python3) as a high level wrapper around \textbf{OpenOCD} (\emph{It allows even to generate \textbf{OpenOCD} scripts on the fly}).\\
...
\begin{enumerate}
	...
	\item \textbf{Interact with the module : } after starting Ftrace, we must call one of the functions of our module. let's make a simple read on it (\textbf{Figure \ref{Interacting with the kernel device module}})
		\begin{figure}[H]
			\centering
        	\includegraphics[scale=0.40]{img/solution/interacting-driver-trace-cmd.png}
        	\caption{Interacting with the kernel device module}
        	\label{Interacting with the kernel device module}
    	\end{figure}		
	
	
	\item \textbf{Reading report : } the trace file can be read as shown in \textbf{Figure \ref{Reading module trace report with Trace-cmd}}.
			\begin{figure}[H]
			\centering
        	\includegraphics[scale=0.40]{img/solution/trace-cmd-report-module.png}
...
Reading Ftrace report on a terminal can be quite difficult for interpretation; the third tool released by \og \textbf{Steven Rostedt} \fg is KernelShark which is GUI parser for trace-cmd's traces (an example is shown in \textbf{Figure \ref{Kernelshark shows process scheduling after parsing traces of a raspberry PI 3}}).
...
An example is shown in \textbf{Figure \ref{Reading recorded sched-switch event - perf}}.
	...
	\item[$\bullet$] {Hotspot : } A \textbf{GUI tool} for \textbf{Perf}, able to parse \og perf traces \fg or even make a entire perf session recording.	An example is shown in \textbf{Figure \ref{Hotspot (a perf GUI tool) in action - generation of flame graphs}}.
		
					\begin{figure}[H]
						\centering
        				\includegraphics[scale=0.4]{img/solution/perf-hotspot.png}
...
\fbox{
  \parbox{\textwidth}{
	...
	{\Large \textbf{\color{red}eBPF enhanced security}}\\
	 \textbf{eBPF} imposes restrictions on how programs should be written (\textit{no infinite loops, kprobes cannot be attached to all functions, ..., etc}). It ensures that a script will never crash or hang kernel code ({\color{blue} \url{https://lkml.org/lkml/2015/4/14/232}}). 
	\end{center} 
  }
}
\end{center}
...
Bugs are not only introduced as a result of programming mistakes (\textit{no one writes perfect code}), they can be caused by malicious code injected on purpose by attackers.
...
Dumping the infected module using \og objdump -t kernel-module-infected.ko \fg is demonstrated in \textbf{Figure \ref{Forcing init-module to become an alias of a malicious function}}

\begin{figure}[H]
					\centering
        			\includegraphics[scale=0.65]{img/solution/infected-module-dumped.png}
...
\item \textbf{Insert infected module into kernel : } After loading the buffer into the kernel, one can see that malicious function has been executed (see \textbf{Figure \ref{Infected module executing malicious function}}).

\begin{figure}[H]
					\centering
        			\includegraphics[scale=0.5]{img/solution/hacked-module.png}
